...
import { isSpecialSeed } from "./createList";
...
export function encodeSave(save: any) {
  ...
  s.levels.forEach((level: Level) => {
    levelsTrimmed.push([
      btoa(level.name),
      level.position.toString(36),
      level.level_id.toString(36),
      level.completed_percentage.toString(36),
    ]);
  });

  s.levels = levelsTrimmed;
  s.seed = isSpecialSeed(s.seed) ? s.seed : s.seed.toString(36);
  ...
  if (s.extra) s.extra = btoa(JSON.stringify(s.extra));
...
export function decodeSave(save: string) {
  ...
  parsedSave.seed = isSpecialSeed(parsedSave.seed)
    ? parsedSave.seed
    : parseInt(parsedSave.seed, 36);
  ...
  if (parsedSave.extra) parsedSave.extra = JSON.parse(atob(parsedSave.extra));

  let levelsExpanded: any[] = [];
  parsedSave.levels.forEach((level: [string, string, string, string]) => {
    levelsExpanded.push({
      name: atob(level[0]),
      position: parseInt(level[1], 36),
      level_id: parseInt(level[2], 36),
      completed_percentage: parseInt(level[3], 36),
    });
  });
